...
local function getPredictedPosition(character)
    ...
    if primaryPart then
        ...
        local predictionMultiplier = getgenv().TriggerBot.Settings.Preds.PredictionMultiplier or 2.0
...
local function adjustAimForTarget(targetPosition)
    ...
    if onScreen then
        ...
        -- Adding smoothing to the aim
        local smoothingFactor = 0.1 -- Adjust this for more/less smoothing
        local offsetX = (screenPosition.X - centerX) * smoothingFactor
        local offsetY = (screenPosition.Y - centerY) * smoothingFactor

        -- Smooth the mouse movement over time
    ...
    if TargetPlayer and TargetPlayer.Character then
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            ...
            if predictedPosition and isWithinBoxFOV(predictedPosition) then
                ...
                if closestPart then
                    -- Implementing the delay smoothing logic
                    local timeSinceLastClick = os.clock() - lastClickTime
                    local triggerDelay = getgenv().TriggerBot.Settings.TriggerDelay or 0.05
                    local maxDelay = getgenv().TriggerBot.Settings.MaxDelay or 0.1
                    local minDelay = getgenv().TriggerBot.Settings.MinDelay or 0.01
                    
                    -- Smooth the delay with random variation
                    local delay = math.random() * (maxDelay - minDelay) + minDelay
                    if timeSinceLastClick >= triggerDelay then
